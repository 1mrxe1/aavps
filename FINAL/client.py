...
@client.on(events.NewMessage(outgoing=True, pattern=r"\.م م م ٤د ٧ "))
async def send_inline_buttons(event):
    global owner_id
    ...
    if event.sender_id != owner_id:
        await event.reply("🚫 قم بتثبيت سورس فاينل للتمكن من ذلك.")
        return

    bot_username = await get_bot_username()
    if not bot_username:
        await event.reply("⚠️ لم يتمكن البوت من جلب يوزره. يرجى إعادة تشغيله.")
        return

    response = await client.inline_query(bot_username, "اوامري")
